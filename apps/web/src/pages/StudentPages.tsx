...
import { useApiAction, useJsonQuery } from '../hooks/useApi'
import './PlatformPages.css'
import { ReportSource } from '../components/ReportSource'
import { SupportReadback } from '../components/SupportReadback'
import { MistakeStudy, StudentReviews } from '../features/education/LearningPanels'

type Dashboard = {
  student: { id: string; name: string }
  latest_exam: { id: string; name: string; date: string | null; total_score: number; full_score: number | null; class_rank: number | null; grade_rank: number | null; score_delta: number | null } | null
  subjects: Array<{ name: string; score: number; full_score: number | null; percentage: number | null; class_rank: number | null; grade_rank: number | null }>
...
type TrendData = { exams: Array<{ id: string; name: string; date: string | null; score: number; full_score: number | null; class_rank: number | null; grade_rank: number | null }>; subjects: Array<{ subject: string; exam: string; date: string | null; score: number; full_score: number | null }>; subject_names: string[] }
type Diagnosis = { has_source: boolean; id: string; type: string; status: string; version: string | null; generated_at: string | null; exam_name: string | null; content: Record<string, unknown>; source: string }
type Mistake = { id: string; exam: string; subject: string; question_no: string; score: number; full_score: number | null; lost_score: number; answer_status: string | null; date: string | null }

function useJson<T>(path: string) {
  // 统一走 TanStack Query（带身份隔离的查询键），不再各自维护请求状态。
  const query = useJsonQuery<T>(path)
  return {
...
function ContentState({ loading, error, empty, onRetry, children }: { loading: boolean; error: Error | null; empty?: boolean; onRetry?: () => void; children: React.ReactNode }) {
  ...
  if (empty) return <div className="empty-state"><div className="empty-state__icon">⌁</div><strong>还没有可展示的数据</strong><p>完成一次考试或导入成绩后，这里会展示已授权且已同步的数据。</p></div>
  return <>{children}</>
}
...
export function StudentDashboardPage() {
  const { data, loading, error, reload } = useJson<Dashboard>('/platform/student/dashboard')
  ...
  return <AppShell title={`你好，${data?.student.name || '同学'}`} eyebrow="学生空间 / 学习总览">
    <ContentState loading={loading} error={error} onRetry={reload}>
      ...
      <div className="content-grid content-grid--wide">
        <section className="panel panel--large"><div className="panel-heading"><div><h2>最近一次考试</h2><p>{exam?.name || '尚未同步考试成绩'}</p></div><span className="panel-heading__date">{exam?.date || '—'}</span></div>{exam ? <div className="score-hero"><div className="score-hero__number">{exam.total_score}<small> / {exam.full_score && exam.full_score > 0 ? exam.full_score : '—'}</small></div><div className="score-hero__meta"><div>班级排名 <strong>第 {exam.class_rank || '—'} 名</strong></div><div>年级排名 <strong>第 {exam.grade_rank || '—'} 名</strong></div></div></div> : <div className="inline-empty">完成考试后会在这里看到总分与排名。</div>}</section>
...
export function StudentTrendsPage() {
  const { data, loading, error, reload } = useJson<TrendData>('/platform/student/trends')
  const exams = data?.exams || []
  const pagination = useClientPagination(exams)
...
export function StudentMistakesPage() {
  const { data, loading, error, reload } = useJson<Mistake[]>('/platform/student/mistakes')
  const [subject, setSubject] = useState('全部科目')
  const [activeMistake, setActiveMistake] = useState('')
  ...
  const pagination = useClientPagination(rows, subject)
  const loss = rows.reduce((sum, item) => sum + item.lost_score, 0)
  return <AppShell title="错题分析" eyebrow="学生空间 / 错题分析"><div className="page-toolbar"><Link className="secondary-button" to="/mistakes/reviews">我的订正与复习</Link></div><ContentState loading={loading} error={error} onRetry={reload} empty={!data?.length}><div className="stats-grid stats-grid--three"><StatCard label="累计丢分" value={`${loss.toFixed(1)} 分`} hint="来自已同步的小题记录" tone="orange" /><StatCard label="错题数量" value={rows.length} hint="按筛选条件统计" tone="purple" /><StatCard label="最高单题丢分" value={`${Math.max(...rows.map(item => item.lost_score), 0)} 分`} hint="建议优先复盘" tone="blue" /></div><div className="filter-row"><select className="filter-select" value={subject} onChange={event => setSubject(event.target.value)}>{subjects.map(item => <option key={item}>{item}</option>)}</select></div><Pagination {...pagination} /><div className="data-table-wrap"><table className="data-table"><thead><tr><th>考试</th><th>科目 / 题号</th><th>得分</th><th>丢分</th><th>答题状态</th><th>建议动作</th></tr></thead><tbody>{pagination.items.map(item => <tr key={item.id}><td><strong>{item.exam}</strong><div className="subtle">{item.date || '—'}</div></td><td>{item.subject} · 第 {item.question_no} 题</td><td>{item.score} / {item.full_score}</td><td><span className="loss-number">-{item.lost_score}</span></td><td><span className="status-pill status-pill--amber">{item.answer_status === 'partial' ? '部分得分' : item.answer_status || '待复盘'}</span></td><td><button className="action-link" onClick={() => setActiveMistake(item.id)}>查看原题 / 加入复盘</button></td></tr>)}</tbody></table></div></ContentState>{activeMistake && <DetailDialog title="原题与复盘" onClose={() => setActiveMistake('')}><MistakeStudy key={activeMistake} id={activeMistake} /></DetailDialog>}</AppShell>
}

export function StudentReviewsPage() {
  return <AppShell title="我的订正与复习" eyebrow="学生空间 / 错题分析"><div className="page-toolbar"><Link className="secondary-button" to="/mistakes">返回错题列表</Link></div><StudentReviews /></AppShell>
}

export function StudentSupportHistoryPage() {
  return <AppShell title="反馈与求助记录" eyebrow="学生空间 / 家长与支持"><div className="page-toolbar"><Link className="secondary-button" to="/support">返回支持中心</Link></div><SupportReadback /></AppShell>
}

export function StudentSupportPage() {
  const auth = useJson<Authorization[]>('/platform/student/authorization')
  const [parentName, setParentName] = useState(''); const [parentPhone, setParentPhone] = useState(''); const [message, setMessage] = useState(''); const [feedback, setFeedback] = useState('not_helpful'); const [supportMessage, setSupportMessage] = useState(''); const action = useApiAction(); const saving = action.isPending
  const createAuthorization = (event: React.FormEvent) => { event.preventDefault(); setMessage(''); action.mutate({ path: '/platform/student/authorization', body: { parent_name: parentName, parent_phone: parentPhone } }, { onSuccess: () => { setMessage('授权申请已生成，请把授权码交给家长确认。'); setParentName(''); setParentPhone('') } }) }
  const submitFeedback = (event: React.FormEvent) => { event.preventDefault(); setMessage(''); action.mutate({ path: '/platform/student/feedback', body: { rating: feedback, note: supportMessage, category: '产品反馈' } }, { onSuccess: () => { setMessage('感谢反馈，运营团队会在工作台跟进。'); setSupportMessage('') } }) }
  const requestHandoff = () => { setMessage(''); action.mutate({ path: '/platform/student/handoffs', body: { reason: '学生主动请求人工协助', priority: 'normal', summary: '学生在支持中心请求老师跟进。' } }, { onSuccess: () => setMessage('已提交人工转接，老师会在工作时间内联系你。') }) }
  return <AppShell title="家长与支持" eyebrow="学生空间 / 家长与支持"><div className="page-toolbar"><Link className="secondary-button" to="/support/records">反馈与求助记录</Link></div><ErrorDisplay error={action.error} title="提交失败" onDismiss={action.reset} variant="banner" /><div className="support-grid"><section className="panel"><div className="panel-heading"><div><h2>家长授权</h2><HelpTip label="授权说明">由你发起，家长确认后才能查看指定内容。</HelpTip></div></div><ContentState loading={auth.loading} error={auth.error} onRetry={() => { void auth.reload() }}><div className="authorization-list">{auth.data?.map(item => <div className="authorization-row" key={item.id}><div><strong>{item.parent_name}</strong><span>{item.parent_phone}</span></div><div><span className={`status-pill status-pill--${item.status === 'active' ? 'green' : 'amber'}`}>{item.status === 'active' ? '已授权' : '待确认'}</span>{item.status === 'pending' && <b className="share-code">{item.share_code}</b>}</div></div>)}</div></ContentState><form className="compact-form" onSubmit={createAuthorization}><label className="form-field"><span>家长称呼</span><input className="form-input" value={parentName} onChange={event => setParentName(event.target.value)} placeholder="例如：张女士" required /></label><label className="form-field"><span>手机号</span><input className="form-input" value={parentPhone} onChange={event => setParentPhone(event.target.value)} placeholder="用于确认身份" required /></label><button className="primary-button" disabled={saving}>生成授权申请</button></form></section><section className="panel"><div className="panel-heading"><div><h2>反馈与求助</h2></div></div>{message && <div className="success-message">{message}</div>}<form className="compact-form" onSubmit={submitFeedback}><label className="form-field"><span>这次体验如何</span><select className="form-select" value={feedback} onChange={event => setFeedback(event.target.value)}><option value="helpful">很有帮助</option><option value="not_helpful">需要改进</option><option value="data_wrong">数据有误</option><option value="inappropriate">内容不合适</option></select></label><label className="form-field"><span>补充说明</span><textarea className="form-textarea" value={supportMessage} onChange={event => setSupportMessage(event.target.value)} placeholder="写下你遇到的问题…" /></label><div className="support-buttons"><button className="secondary-button" type="submit" disabled={saving}>提交反馈</button><button className="primary-button" type="button" onClick={requestHandoff} disabled={saving}>联系老师 ↗</button></div></form></section></div></AppShell>
}
