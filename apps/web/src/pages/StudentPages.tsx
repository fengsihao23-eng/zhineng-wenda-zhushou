...
export function StudentMistakesPage() {
  ...
  return <AppShell title="错题分析" eyebrow="学生空间 / 错题分析"><ContentState loading={loading} error={error} onRetry={reload} empty={!data?.length}><div className="stats-grid stats-grid--three"><StatCard label="累计丢分" value={`${loss.toFixed(1)} 分`} hint="来自已同步的小题记录" tone="orange" /><StatCard label="错题数量" value={rows.length} hint="按筛选条件统计" tone="purple" /><StatCard label="最高单题丢分" value={`${Math.max(...rows.map(item => item.lost_score), 0)} 分`} hint="建议优先复盘" tone="blue" /></div><div className="filter-row"><select className="filter-select" value={subject} onChange={event => setSubject(event.target.value)}>{subjects.map(item => <option key={item}>{item}</option>)}</select><span className="subtle">共 {rows.length} 条小题记录</span></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>考试</th><th>科目 / 题号</th><th>得分</th><th>丢分</th><th>答题状态</th><th>建议动作</th></tr></thead><tbody>{rows.map(item => <tr key={item.id}><td><strong>{item.exam}</strong><div className="subtle">{item.date || '—'}</div></td><td>{item.subject} · 第 {item.question_no} 题</td><td>{item.score} / {item.full_score}</td><td><span className="loss-number">-{item.lost_score}</span></td><td><span className="status-pill status-pill--amber">{item.answer_status === 'partial' ? '部分得分' : item.answer_status || '待复盘'}</span></td><td><button className="action-link" onClick={() => setActiveMistake(item.id)}>查看原题 / 加入复盘</button></td></tr>)}</tbody></table></div></ContentState>{activeMistake && <MistakeStudy key={activeMistake} id={activeMistake} />}<StudentReviews /></AppShell>
}
